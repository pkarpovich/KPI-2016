...
namespace LT
{
	...
	{
		if (size > LT_MAXSIZE)	throw GET_ERROR(200)
		else
		{
	...
	{
		if (lextable.size > lextable.maxsize)	throw GET_ERROR(201,4)
		else	lextable.table[lextable.size++] = entry;
	}

	void AddTo(LexTable& lextable, Entry entry, int pos)
	{
		for (int i = lextable.size; i > pos; i--)	lextable.table[i] = lextable.table[i - 1];
	...
	void ShowLT(LexTable l, Parm::PARM param, Log::LOG log)
	{
		...
		for (int i = 0; i < l.size; i++)	//вывод таблицы лексем
		{
			if (l.table[i].lexema != LEX_FORBIDDEN)
			{
				...
				if (l.table[i].idxTI != TI_NULLIDX) { sprintf_s(buf, 255, "{%d}", l.table[i].idxTI); DW(param.LT, buf); }
			}
		}
		DW(param.LT, "\n\n");
		buf = nullptr;
